...
f = open('output_CH_other_native.dat','w')
...
#path_file_org = sys.argv[1]
		...
		collapsed_org = original_org[:] #EXCLUDES THE FINAL ONE. THIS IS HARDCODED.
		#nupY_extended_org = nupY_original_org[612:]

		full_filepath=dirname+ '/' + files

		prot_name_full=full_filepath.split('one-line-sequences/')[1]
		prot_name=prot_name_full.split('-domain')[0]
		...
		f.write('%s\t%.5f\t%2.0f\t%.5f\n' % (prot_name+'   ', ch_ratio_or, nc_old, ncpr_old) )
