...
nupY_original_org = getseq(path_file_org,1,1)
nupY_collapsed_org = nupY_original_org[:] #EXCLUDES THE FINAL ONE. THIS IS HARDCODED.
#nupY_extended_org = nupY_original_org[612:]


prot_name_full=path_file_org.split('one-line-sequences/')[1]

hydrophobicities_or,indices_or,charges_or = calculate_hydro_charge(nupY_collapsed_org)
h_dens_or,c_dens_or,ch_ratio_or = calculate_CH_ratio(nupY_collapsed_org)
nc_old,ncpr_old = calculate_net_charge(nupY_collapsed_org)

#h_dens_ext_or, c_dens_ext_or, ch_ratio_ext_or = calculate_CH_ratio(nupY_extended_org)

print(prot_name, ch_ratio_or)
