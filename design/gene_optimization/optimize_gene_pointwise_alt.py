...
os.system("grep -v '>' %s > tmp.txt" % (path_file))
os.system("tr -d '\n' < tmp.txt > tmp2.txt")
...
def pick_alternative_codon(seq, position_index,tableC=codon_table,tableA=AA_table):
...
aa_numbers_to_change = [79, 82, 83, 94, 106, 108, 111, 112, 114, 131, 143, 172, 180, 183, 185, 192, 197, 203, 208, 218, 221, 229, 241, 255, 261, 267, 269, 270, 279, 284, 290, 294, 310, 317, 330, 332, 336, 338, 341, 347, 372, 379, 388, 399, 403, 411, 427, 457, 460, 469, 477, 482, 493, 497, 499, 508, 511, 521, 526, 533, 539, 546, 550, 593, 594, 597, 599, 614, 617, 626, 628, 635]
codon_numbers_to_change  = [3*aa_num for aa_num in aa_numbers_to_change]

#codon_numbers_to_change  = range(2009,2026+1)
#codon_numbers_to_change2 = range(1036,1063+1)
#codon_numbers_to_change3 = range(2086,2121+1)
#codon_numbers_to_change4 = range(1348,1388+1)
#codon_numbers_to_change5 = range(1801,1814+1)
...
#nupy_gene = getseq(path_file,2,1)
nupy_aa   = seq_from_gene(nupy_gene)

for number in start_numbers:
	nupy_gene = pick_alternative_codon(nupy_gene,number)
