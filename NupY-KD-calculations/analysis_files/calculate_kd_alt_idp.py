...
max_int_filename = "pairdist_prot_2_max.xvg"
...
def calc_dee(filename_list):
	...
	dee_values = np.zeros(len(filename_list))
	dee_std    = np.zeros(len(filename_list))
	...
	for index,file_name in enumerate(filename_list):
		...
		#layout of polystat.xvg is typically: time / D_ee / Rg, so we use column three, which is no. 2 in zero-based indexing.
		dee_array = read_file(file_name,1)
   
		mean_dee = np.mean(dee_array)
		std_value = np.std(dee_array)
		dee_values[index] = mean_dee
		dee_std[index] = std_value


	return dee_values, dee_std


def calc_dmax(filename_list):
	'''
	calc_rg(filename_list): This subroutine calculates the Rg and associated standard deviation for protein_1 and protein_2.
	This assumes that you prepared two polystat.xvg files without headers.
	POlystat files are indexed as:
	...
	dmax_values = np.zeros(len(filename_list))
	dmax_std    = np.zeros(len(filename_list))
	...
	for index,file_name in enumerate(filename_list):
		...
		#layout of the file is: two columns, timestep / dmax

		dmax_array = read_file(file_name,1)
   
		mean_dmax = np.mean(dmax_array)
		std_value = np.std(dmax_array)
		dmax_values[index] = mean_dmax
		dmax_std[index] = std_value


	return dmax_values, dmax_std
...
def calc_subvolume_radius_idp(protein_filename, prot_length_1, prot_length_2, dee_prot_2):
	...
	D_1 = np.max(d_prot_1)
	D_2 = dee_prot_2*10 #nm to Angstrom
...
dmax_values,dmax_std = calc_dmax(filename_list = [max_int_filename])
#rg_upper_lim = [rg+std for rg,std in zip(rg_values,rg_std)]
#rg_lower_lim = [rg-std for rg_std in zip(rg_values,rg_std)]

R_subvolume = calc_subvolume_radius_idp(input_struct,prot_1_size,prot_2_size,dmax_values)
R_subvolume_upper = calc_subvolume_radius_idp(input_struct,prot_1_size,prot_2_size,dmax_values+dmax_std)
R_subvolume_lower = calc_subvolume_radius_idp(input_struct,prot_1_size,prot_2_size,dmax_values-dmax_std)
...
str_pre = os.getcwd().split('results/NupY_')[1]
CH_val= float(str_pre.split('_')[1])
dFG_val= float(str_pre.split('_')[0])

f = open('kd.dat','w+')


f.write('%.3f\t%.3f\t%f\t%f\t%f\t%f\t%f\t%f\n' % (dFG_val, CH_val, kd*10**6, kd_upper*10**6, kd_lower*10**6, bound_fraction, subvolume_fraction,V_subvolume/V_box))
f.close()
